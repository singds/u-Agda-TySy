...
Proving that this is not the case is equivalent to proving that for any environment $\Gamma$ and any type $T$ the judgement $\Gamma \vdash M : T$ is not derivable.
The proposition $\forall \; \Gamma \in \text{Env} \; \forall \; T \in \text{Type} \; \neg (\Gamma \vdash M : T \text{ is derivable})$ is represented by the type \texttt{(Γ:Env) → (T:Type) → HasType Γ M T → ⊥}.
