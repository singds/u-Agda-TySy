...
{-
no-fv-instance : (x : ℕ) (N M : Term) → ¬ (x ∈ fv N) → ¬ (x ∈ fv (subst x N M))
no-fv-instance x N (var y) p with y ≡? x
... | left  p1 = p
... | right p1 = λ p2 → p1 (symm (is-the-first p2))
no-fv-instance x N (M1 app M2) p = {!!}
no-fv-instance x N (fun t M) p = {!!}
-}
